...
if __name__ == "__main__":
    ...
    # Database mein findings daalo
    from database import db
    import json
    
    print("\n" + "=" * 60)
    print("📊 Storing Keycloak findings in database")
    print("=" * 60)
    ...
    try:
        ...
        if total_users > 0:
            ...
            if mfa_rate < 0.8:
                severity = "critical" if mfa_rate < 0.3 else "high" if mfa_rate < 0.5 else "medium"
                
                import uuid
                finding_id = f"keycloak_mfa_{uuid.uuid4().hex[:8]}"
                
                db.execute_query(
                    """INSERT INTO findings (finding_id, asset_id, source, title, description, severity, raw_data)
                       VALUES (%s, %s, %s, %s, %s, %s, %s)
                       ON CONFLICT (finding_id) DO NOTHING""",
                    (finding_id, 'identity_service', 'keycloak',
                     f'Low MFA Adoption Rate ({mfa_rate*100:.0f}%)',
                     f'{total_users} users, {mfa_enabled} with MFA',
                     severity,
                     json.dumps(data))
                )
                print(f"✅ Keycloak finding stored: MFA rate {mfa_rate*100:.0f}%")
    except Exception as e:
        print(f"❌ Keycloak ingestion failed: {e}")
    finally:
        ingestor.close()